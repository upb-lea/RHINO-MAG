...
class JAWithExternGRUwInterface(ModelInterface):
    ...
    def __call__(self, B_past, H_past, B_future, T, warmup = True):
        B_all = jnp.concatenate([B_past, B_future], axis=1)
        B_all_norm, H_past_norm, T_norm = self.normalizer.normalize(B_all, H_past, T)
        B_past_norm = B_all_norm[:, : B_past.shape[1]]
        B_future_norm = B_all_norm[:, B_past.shape[1] :]
    ...
    def normalized_call(self, B_past_norm, H_past_norm, B_future_norm, T_norm,warmup = True):
        ...
        batch_x = jnp.concatenate([batch_H_pred_norm_ja[..., None],B_future_norm[..., None], T_norm_broad[..., None], features_norm], axis=-1)
    ...
    def _warmup(self, B_past_norm, H_past_norm, B_future_norm, T_norm):
        ...
        batch_x = jnp.concatenate(
            [batch_H_pred_norm_ja[..., None],B_past_norm[:, 1:, None], T_norm_broad[:, 1:, None], features_norm[:, 1:]], axis=-1
        )
        init_hidden = jnp.zeros((H_past_norm.shape[0], self.model.gru.hidden_size))
...
class GRUWithJAwInterface(ModelInterface):
    model: eqx.Module
    ...
    def normalized_call(self, B_past_norm, H_past_norm, B_future_norm, T_norm):
        ...
        init_hidden = jnp.hstack(
            [H_past_norm[:, -1, None],jnp.zeros((H_past_norm.shape[0], self.model.gru.hidden_size - 1))]
        )
    ...
    def normalized_warmup_call(self, B_past_norm, H_past_norm, B_future_norm, T_norm):
        ...
        init_hidden = jnp.hstack(
            [H_past_norm[:, 0, None],jnp.zeros((H_past_norm.shape[0], self.model.gru.hidden_size - 1))]
        )
...
class LFRWithGRUJAwInterface(ModelInterface):
    model: eqx.Module
    normalizer: eqx.Module
    featurize: Callable = eqx.field(static=True)
    ...
    def normalized_call(self, B_past_norm, H_past_norm, B_future_norm, T_norm):
        ...
        B_curr_and_future_norm = B_all_norm[:, B_past_norm.shape[1] - 1 :] 
        H0_norm = H_past_norm[:, -1]
        ...
        batch_H_pred_norm = jax.vmap(single_batch)(H0_norm, B_curr_and_future_norm, batch_x, init_hidden)
        #batch_H_pred_norm = jax.vmap(jax.vmap(self.normalizer.normalize_H))(batch_H_pred)
        return batch_H_pred_norm
