# Author: fddf
# source: https://github.com/fddf22/Preisachmodel
...
def analyticalPreisachFunction1(
    a: float, b: float, c: float, d: float, n: float, p: float, q: float, beta: np.ndarray, alpha: np.ndarray
) -> np.ndarray:
    """
...
class PreisachModel:
    ...
    def showEverettFunction(self, fig: plt.Figure) -> None:
        ...
        ax.plot_surface(self.gridX, self.gridY, Z)
    ...
    def showInterface(self, fig: plt.Figure) -> None:
        ...
        ax.plot(
            np.array([-self.beta0, self.beta0, -self.beta0, -self.beta0]),
            np.array([-self.alpha0, self.alpha0, self.alpha0, -self.alpha0]),
            linewidth=3,
        )
    ...
    def animateHysteresis(self) -> animation.FuncAnimation:
        ...
        simulation = animation.FuncAnimation(
            fig1, update_line, frames, fargs=(self, line1, line2, line3), interval=25, blit=True, repeat=False
        )
