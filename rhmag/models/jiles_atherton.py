...
class LFRWithGRUJA(eqx.Module):
    ...
    def __init__(self, normalizer, key, in_size, hidden_size=8, x_k_size=5, **kwargs):
        ...
        intercon_matrix_init = (
            jax.random.uniform(key, (x_k_size + 1 + 3 + in_size, x_k_size + (3 + n_features) + 1 + 1)) * 0.1
        )  # (x_k_size + y_size + ja_in_size + gru_in_size, x_k_size + u_k_size + output_ja_size + output_gru_size)
