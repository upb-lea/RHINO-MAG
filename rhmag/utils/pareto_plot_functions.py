...
def visualize_pareto_front_average(
    df,
    df_external,
    metrics,
    colors,
    ...
    for i, metric in enumerate(metrics):
        ...
        for color, group_df in df.groupby("color"):
            ...
            avg_df = group_df.groupby("model_type").median(numeric_only=True)
            sorted_indices = np.argsort(avg_df.n_params)
            avg_df = avg_df.iloc[sorted_indices]

            avg = avg_df[f"{metric}_95th"]
            ax.plot(avg, avg_df["n_params"], c=current_color, alpha=0.8, marker="x", ms=3)
        # End own results
